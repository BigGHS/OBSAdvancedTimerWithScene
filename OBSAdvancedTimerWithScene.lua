...
function set_time_text(ns, text)
	...
	if mode == "Countdown" or mode == "Specific time" or mode == "Specific date and time" then
        if up_when_finished == true and up == true then text = stop_text .. text end
        if text ~= stop_text and up ~= true then text = start_text .. text end
    end
...
function settings_modified(props, prop, settings)
	...
	local p_last_text = obs.obs_properties_get(props, "last_text")
	local p_start_text = obs.obs_properties_get(props, "start_text")
	local p_stop_text = obs.obs_properties_get(props, "stop_text")
	local p_next_scene = obs.obs_properties_get(props, "next_scene")
	...
	if (mode_setting == "Countdown") then
		...
		obs.obs_property_set_visible(p_last_text, true)
		obs.obs_property_set_visible(p_start_text, true)
		obs.obs_property_set_visible(p_stop_text, true)
		obs.obs_property_set_visible(p_next_scene, true)
	...
	elseif (mode_setting == "Countup") then
		...
		obs.obs_property_set_visible(p_minutes, false)
		obs.obs_property_set_visible(p_seconds, false)
		obs.obs_property_set_visible(p_start_text, false)
		obs.obs_property_set_visible(p_stop_text, false)
		obs.obs_property_set_visible(p_next_scene, false)
		obs.obs_property_set_visible(button_pause, true)
		obs.obs_property_set_visible(button_reset, true)
		obs.obs_property_set_visible(p_a_mode, true)
...
function script_properties()
	...
	local t = obs.obs_properties_add_list(props, "next_scene", "-or- Next Scene", obs.OBS_COMBO_TYPE_EDITABLE, obs.OBS_COMBO_FORMAT_STRING)
	...
	obs.obs_properties_add_bool(props, "countup_countdown_finished", "Countup when done (overrides Next Scene)")
...
function script_defaults(settings)
	...
	obs.obs_data_set_default_string(settings, "start_text", "Starting in: ")
	obs.obs_data_set_default_string(settings, "stop_text", "Live shortly! ")
